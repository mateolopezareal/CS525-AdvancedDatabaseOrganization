...
#define ASSERT_EQUALS_RECORDS(_l, _r, schema, message)            \
        do {                                    \
            ...
            {                                    \
                ...
                if (lVal->dt == DT_STRING){                        \
                    free(lVal->v.stringV);             \
                }                                                 \
                if (rVal->dt == DT_STRING){                        \
                    free(rVal->v.stringV);             \
                }\
                free(lVal);                                       \
                free(rVal); \
...
testRecords(void) {
    ...
    Value * toCompare = stringToValue("i1");
    OP_TRUE(toCompare, value, valueEquals, "first attr");
    free(toCompare);
    freeVal(value);

    toCompare = stringToValue("saaaa");
    getAttr(r, schema, 1, &value);
    OP_TRUE(toCompare, value, valueEquals, "second attr");
    freeVal(toCompare);
    freeVal(value);

    toCompare = stringToValue("i3");
    getAttr(r, schema, 2, &value);
    OP_TRUE(toCompare, value, valueEquals, "third attr");
    free(toCompare);
    freeVal(value);

    //modify attrs
    toCompare = stringToValue("i4");
    Value * toSet = stringToValue("i4");
    setAttr(r, schema, 2, toSet);
    getAttr(r, schema, 2, &value);
    OP_TRUE(toCompare, value, valueEquals, "third attr after setting");
    free(toSet);
    free(toCompare);
    freeVal(value);
...
testCreateTableAndInsert(void) {
    ...
    for (i = 0; i < 1000; i++) {
        ...
        Record * toCompare = fromTestRecord(schema, inserts[pos]);
        ASSERT_EQUALS_RECORDS(toCompare, r, schema, "compare records");
        freeRecord(toCompare);
    }
    freeRecord(r);  // Addedw: Summer 2021
...
testUpdateTable(void) {
    ...
    for (i = 0; i < numFinal; i++) {
        RID rid = rids[i];
        TEST_CHECK(getRecord(table, rid, r));
        Record * toCompare = fromTestRecord(schema, finalR[i]);
        ASSERT_EQUALS_RECORDS(toCompare, r, schema, "compare records");
        freeRecord(toCompare);
    }
...
testInsertManyRecords(void) {
    ...
    Record * toTest = fromTestRecord(schema, updates[0]);
    ASSERT_EQUALS_RECORDS(toTest, r, schema, "compare records");
    freeRecord(toTest); // added by student -
    freeRecord(r);
...
void testScans(void) {
    ...
    while ((rc = next(sc, r)) == RC_OK) {
        for (i = 0; i < scanSizeOne; i++) {
            Record * toCompare = fromTestRecord(schema, scanOneResult[i]);
            if (memcmp(toCompare->data, r->data, getRecordSize(schema)) == 0)
                foundScan[i] = TRUE;
            freeRecord(toCompare);
        }
    }
...
void testScansTwo(void) {
    ...
    TEST_CHECK(startScan(table, sc, sel));
    while ((rc = next(sc, r)) == RC_OK) {
        Record * toCompare = fromTestRecord(schema, inserts[1]);
        ASSERT_EQUALS_RECORDS(toCompare, r, schema, "compare records");
        freeRecord(toCompare);
    }
    if (rc != RC_RM_NO_MORE_TUPLES)
    ...
    TEST_CHECK(startScan(table, sc, sel));
    while ((rc = next(sc, r)) == RC_OK) {
        Record * toCompare = fromTestRecord(schema, inserts[5]);
        ASSERT_EQUALS_RECORDS(toCompare, r, schema, "compare records");
        freeRecord(toCompare);
    }
    ...
    while ((rc = next(sc, r)) == RC_OK) {
        for (i = 0; i < numInserts; i++) {
            Record * toCompare = fromTestRecord(schema, inserts[i]);
            if (memcmp(toCompare->data, r->data, getRecordSize(schema)) == 0)
                foundScan[i] = TRUE;
            freeRecord(toCompare);
        }
